...
class BlogServices {
  ...
  Future<void> createBlog({
    ...
    try {
      ...
      request.files.add(http.MultipartFile.fromBytes(
        ...
        // Ajustez le type de contenu ici en fonction du type de fichier réel
        contentType: MediaType('image', 'png'), // Exemple : changez en 'image/jpeg' si c'est un fichier JPEG
      ));
  ...
  Future<Image> loadImageFromPath(String imagePath) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return Image.memory(
          ...
          fit: BoxFit.contain, // Essayez différentes options de fit
        );
      } else {
        throw Exception('Failed to load image from path. Status code: ${response.statusCode}');
      }
    } catch (e) {
      throw Exception('Error loading image from path: $e');
    }
  }
  ...
  Future<void> deleteBlog(String blogId) async {
    try {
      // Ensure that the blogId is not null or empty
      if (blogId == null || blogId.isEmpty) {
        throw Exception('Invalid blog ID');
      }
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to delete blog. Status Code: ${response.statusCode}, Response Body: ${response.body}');
      }
    } catch (e) {
      throw Exception('Error deleting blog: $e');
    } finally {
      await fetchBlogList(); // Ensure that fetchBlogList is called regardless of success or failure
    }
  }

  Future<List<Comment>> getCommentsByBlogId(String blogId) async {
    final response = await http.get(Uri.parse('$apiUrl/comments?blogId=$blogId'));
